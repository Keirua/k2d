...
{
	K2D_Log* pLog = K2D_Log::GetInstance();
	if( SDL_Init( SDL_INIT_VIDEO ) < 0 ) 
	{
        pLog->Log (K2D_ERROR, "Couldn't Loadialize SDL: %s", SDL_GetError());
	...
	{
		pLog->Log (K2D_ERROR,"SDL_SetVideoMode(): %s", SDL_GetError());
		return 0;
	}
	...
	pLog->Log (K2D_OK, "Window successfully opened");
	return true;
}
