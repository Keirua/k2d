...
//création d'une image : 
	...
	ilGenImages(1, &id); 
	ilBindImage(id); 
	ilTexImage(m_Window.GetWidth(), m_Window.GetHeight(), 1, 3, IL_RGB, IL_UNSIGNED_BYTE, NULL); 
 
//copie du framebuffer d'OpenGL actuellement affiché vers l'image : 
	glReadPixels(0, 0, m_Window.GetWidth(), m_Window.GetHeight(), GL_RGB, GL_UNSIGNED_BYTE, ilGetData()); 
	...
	while( SDL_PollEvent( &m_Event ) )
	{
		switch( m_Event.type )
		{
			...
			case SDL_KEYDOWN:
				...
				if (m_Event.key.keysym.sym == SDLK_ESCAPE || m_Console.isEnabled() == false)
				{
